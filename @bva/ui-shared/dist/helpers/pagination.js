...
const getPrev = (current, start) => {
  return current <= start ? null : current - 1;
};

const getNext = (current, end) => {
  return current >= end ? null : current + 1;
};

const getPageList = (total = 0) => {
  return Array.from(Array(total), (_, i) => i + 1);
};

const getVisiblePageList = (list, current = 1, total, visible = 5) => {
  if (total <= visible) {
    return list;
...
const getPagination = (current, total, visible) => {
  const pagination = {
...
exports.getVisiblePageList = getVisiblePageList;
