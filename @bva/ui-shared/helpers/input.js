 ...
 * @return {Array}
 */
export const getNumberedList = (min = 0, max = 50, step = 1) => {
 ...
 */
export const getConstrainedValue = (value, min, max) => {
 ...
 */
export const getSelectedOptionLabel = (target) => {
...
/**
 * Toggles the selected or checked state of a group of radio buttons
 * @param {Object} selectedRadioOption the selected radio button config object. 
 * @param {Array} radioOptions array of all radio option config objects
 * @returns 
 */
export const updateSelectedRadioOption = (selectedRadioOption, radioOptions) => {
  return radioOptions.map((radioOption) => ({
    ...radioOption,
    checked: radioOption.name === selectedRadioOption.name,
  }));
};

const inputMaskConfigs = {
  shortDate: {
    date: true,
    delimiter: '/',
    datePattern: ['m', 'y']
