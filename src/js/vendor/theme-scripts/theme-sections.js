...
class Section {
  constructor(container, registration) {
    ...
    this.id = container.getAttribute(attributes.id);
...
function validateContainerElement(container) {
  ...
  if (container.getAttribute(attributes.id) === null) {
    throw new Error('Theme Sections: The section container provided does not have an id assigned to the ' + attributes.id + ' attribute.');
  }
...
function load(types, containers) {
  ...
  if (typeof containers === 'undefined') {
    containers = document.querySelectorAll('[' + attributes.type + ']');
  }
  ...
  types.forEach(function (type) {
    ...
    containers = containers.filter(function (container) {
      ...
      if (container.getAttribute(attributes.type) === null) {
        return false;
      }

      // Keep in list of containers because current type doesn't match
      if (container.getAttribute(attributes.type) !== type) {
        return true;
      }
...
if (window.Shopify.designMode) {
  document.addEventListener('shopify:section:load', function (event) {
    ...
    var container = event.target.querySelector('[' + attributes.id + '="' + id + '"]');
    ...
    if (window.Shopify.visualPreviewMode === true) {
      ...
      if (container === null) {
        ...
        container = event.target.querySelector(`[${attributes.id}]`);
      }
    }

    if (container !== null) {
      load(container.getAttribute(attributes.type), container);
    }
  });
