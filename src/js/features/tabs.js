...
class GlobalTabs {
  ...
  init() {
    ...
    if (tabsNavList.length) {
      tabsNavList.forEach((element) => {
        ...
        const tab = this.container.querySelector(`${selectors.tabContent}-${tabId}`);
        ...
        element.addEventListener('keyup', (event) => {
          if ((event.code === 'Space' || event.code === 'Enter') && this.body.classList.contains(classes.focused)) {
            this.tabChange(element, tab);
          }
        });
      });
  ...
  tabChange(element, tab) {
    ...
    const currentTab = this.container.querySelector(`${selectors.tabsLi}.${classes.current}`);
    const currentTabContent = this.container.querySelector(`${selectors.tabContent}.${classes.current}`);
